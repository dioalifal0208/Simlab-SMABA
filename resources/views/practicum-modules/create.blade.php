                    ...
                    <form action="{{ route('practicum-modules.store') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="title" id="title" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-smaba-dark-blue focus:ring-smaba-dark-blue" value="{{ old('title') }}" required autofocus>
                            </div>

                            {{-- Deskripsi / Langkah-langkah --}}
                            <div>
                                <label for="description" class="block font-medium text-sm text-gray-700">Deskripsi / Langkah-langkah (Opsional)</label>
                                <textarea name="description" id="description" rows="5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-smaba-dark-blue focus:ring-smaba-dark-blue" placeholder="Tuliskan langkah-langkah praktikum atau SOP di sini...">{{ old('description') }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="border rounded-lg p-4 bg-gray-50 max-h-72 overflow-y-auto">
                                    <input type="text" id="item-search-module" placeholder="Ketik untuk mencari item..." class="w-full mb-4 pl-4 pr-4 py-2 rounded-md border-gray-300 shadow-sm focus:border-smaba-dark-blue focus:ring-smaba-dark-blue text-sm">
                                    <div class="space-y-2" id="item-list-module">
                                        @forelse ($items as $item)
                                            <div class="flex items-center item-entry-module">
                                                <input type="checkbox" name="items[]" value="{{ $item->id }}" id="module_item_{{ $item->id }}" class="h-4 w-4 rounded border-gray-300 text-smaba-dark-blue focus:ring-smaba-light-blue" {{ in_array($item->id, old('items', [])) ? 'checked' : '' }}>
                        ...
                        <div class="mt-8 flex justify-end space-x-3">
                            ...
                            <button type="submit" class="px-6 py-2 bg-smaba-dark-blue text-white rounded-md hover:bg-smaba-light-blue font-semibold text-sm shadow-md transition-colors">Simpan Modul</button>
                        </div>
                    </form>
                </div>
            </div>
