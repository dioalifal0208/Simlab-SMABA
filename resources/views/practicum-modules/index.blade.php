    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            ...
            @can('manage-documents') {{-- Menggunakan gate yang sama dengan Pustaka Digital --}}
                <a href="{{ route('practicum-modules.create') }}" class="mt-3 sm:mt-0 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold text-sm shadow-sm transition-colors flex items-center gap-2">
                            ...
                            @forelse ($modules as $module)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="flex items-center justify-center space-x-2">
                                        <a href="{{ route('practicum-modules.show', $module->id) }}" class="px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-semibold text-xs shadow-sm transition-colors duration-300">
                                            Detail
                                        </a>

                                        @can('manage-documents') {{-- Asumsi gate yang sama --}}
                                            {{-- TOMBOL EDIT BARU --}}
                                            <a href="{{ route('practicum-modules.edit', $module->id) }}" class="p-1.5 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 shadow-sm transition-colors duration-300" title="Edit">
                                            ...
                                            <form action="{{ route('practicum-modules.destroy', $module->id) }}" method="POST" class="delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-1.5 bg-red-600 text-white rounded-lg hover:bg-red-700 shadow-sm transition-colors duration-300" title="Hapus">
