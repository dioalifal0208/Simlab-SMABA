                    ...
                    <div class="bg-white overflow-hidden shadow-sm border border-gray-100 sm:rounded-xl">
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-4">
                                    <div><label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Pemohon</label><p class="font-semibold text-gray-800 text-lg">{{ $booking->user->name }}</p></div>
                                    <div><label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Tanggal Pengajuan</label><p class="text-gray-700">{{ $booking->created_at->format('d F Y, H:i') }}</p></div>
                                ...
                                <div class="md:col-span-2 pt-4 border-t border-gray-100">
                                    <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Tujuan Kegiatan</label>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm border border-gray-100 sm:rounded-xl">
                        <div class="p-6 text-center">
                            <h3 class="text-gray-500 font-medium text-sm uppercase tracking-wider mb-4">Status Saat Ini</h3>
                            ...
                            @elseif($booking->status == 'approved') 
                                ...
                                <p class="text-sm text-gray-500 mt-2">Silakan cetak surat izin</p>
                            ...
                            @elseif($booking->status == 'completed')
                                ...
                                <p class="text-sm text-gray-500 mt-2">Kegiatan telah berakhir</p>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="p-5 space-y-4">
                            @if($booking->status == 'pending')
                                <form action="{{ route('bookings.update', $booking->id) }}" method="POST" class="space-y-4">
                                    @csrf @method('PATCH')
                                    <div>
                                        <label for="admin_notes" class="block text-xs font-medium text-gray-500 uppercase mb-1">Catatan (Review)</label>
                                        <textarea name="admin_notes" id="admin_notes" rows="2" class="block w-full text-sm rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Berikan alasan jika menolak..."></textarea>
                                    </div>
                            ...
                            @elseif($booking->status == 'approved')
                                <form action="{{ route('bookings.update', $booking->id) }}" method="POST">
                                    ...
                                    <p class="text-xs text-gray-400 mt-2 text-center">Klik jika kegiatan & pengembalian sudah beres.</p>
                                </form>
                            @endif
                            ...
                            @if(in_array($booking->status, ['pending', 'rejected', 'completed']))
                                <div class="border-t border-gray-200 pt-4 mt-2">
                                    <form action="{{ route('bookings.destroy', $booking->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus permanen?');">
                                        @csrf @method('DELETE')
